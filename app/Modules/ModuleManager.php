...
class ModuleManager
{
    ...
    {
        return $this->modules->first(fn ($m) => $m->getName() === $name);
    }

    public function has(string $name): bool
    {
        return $this->modules->contains(fn ($m) => $m->getName() === $name);
    }
    ...
    protected function loadModules(): void
    {
        ...
        // Load from app-modules directory (internachi/modular pattern)
            ...
            $module = new class($moduleClass) implements ModuleInterface
            {
                ...
                public function __construct(private string $moduleClass)
                {
                    ...
                    try {
                        ...
                        $name = method_exists($instance, 'getName') ? $instance::getName() : '';
                        $record = Module::where('name', $name)->first();
                        $this->enabled = $record ? $record->enabled : false;
                    } catch (\Throwable) {
                    }
                }
    ...
    protected function hasDependents(string $moduleName): bool
    {
        return $this->enabled()->contains(
            fn ($m) => in_array($moduleName, $m->getDependencies())
        );
    }
    ...
    {
        return $this->modules->map(fn ($m) => $this->getModuleInfo($m->getName()))->toArray();
    }
